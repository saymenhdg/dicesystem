...
const OrderCardPage = ({ userData, cardDesigns, onOrderCard }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err?.detail || err?.message || 'Unable to place card order');
    } finally {
      setSubmitting(false);
    }
  };
                    ...
                    <div className={`relative w-full h-48 bg-gradient-to-br ${design.gradient} rounded-xl p-6 text-white shadow-xl`}>
                      {design.popular && (
                        <div className="absolute inset-x-0 top-3 flex justify-center pointer-events-none">
                          <span className="px-4 py-1.5 bg-yellow-400/95 text-yellow-900 text-xs font-bold uppercase rounded-full shadow-lg">
                            Popular
                          </span>
                        </div>
                      )}
