...
const TransferPage = ({ contacts = [], accountInfo, onTransferComplete }) => {
    ...
    const loadRecipients = async () => {
      try {
        ...
        setRecipients((prev) => {
          ...
          prev.forEach((r) => {
            byUserId.set(String(r.userId), r);
          });
          data.forEach((rec) => {
            const key = String(rec.recipient_id);
            if (!byUserId.has(key)) {
              byUserId.set(key, {
                id: `recipient-${rec.id}`,
                userId: rec.recipient_id,
  ...
  const handleAddRecipient = async ({ lookup, nickname, selectedUser }) => {
    ...
    try {
      // Optional early validation; add_recipient will validate again.
      await verifyRecipient(value).catch(() => null);
      const saved = await addRecipient({ lookup: value, nickname: nickname?.trim() || undefined });
      const item = {
        id: `recipient-${saved.id}`,
        userId: saved.recipient_id,
        name: saved.nickname || saved.username,
        account: saved.username,
      };
      setRecipients((prev) => {
        if (prev.some((r) => String(r.userId) === String(item.userId))) return prev;
        return [...prev, item];
      });
      setRecipientId(String(saved.recipient_id));
